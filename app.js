const routeCatalog = [
  {
    ...
    stops: [
      ...
      },
      {
        ...
        output: "1 Blogpost mit Gegenrede.",
        source: "Enzensberger"
      },
      {
        title: "Selfie-Point: Abgefahren 4+5",
  ...
  },
  {
    ...
    stops: [
      ...
      },
      {
        ...
        encounter: "Triadisches Streitformat.",
        output: "These + Antithese.",
        source: "Enzensberger"
      },
      {
        title: "Selfie-Point",
        subtitle: "Abgefahren Kap. 4",
        focus: "Beschleunigung als Selbstbild.",
        textLoop: "Kap. 4 zweimal, mit Perspektivwechsel lesen.",
        tasks: [
          "Bewegung als Statusmarker belegen.",
          "Erholungsversprechen gegen Leistungsdruck spiegeln.",
          "Flut-Korrespondenz dokumentieren."
        ],
      ...
      },
      {
        ...
        subtitle: "Abgefahren Kap. 6",
        focus: "Leere als Infrastrukturprodukt.",
        textLoop: "Kap. 6 in drei Lesegeschwindigkeiten.",
        tasks: [
          "Leere-Mechanik in 5 Schritten darstellen.",
      ...
      {
        title: "Retro-Motel",
        subtitle: "Abgefahren Kap. 7",
        focus: "Nostalgie als moderne Betriebsform.",
  ...
  {
    id: "distance",
    name: "Distance-Nomadenroute",
    profile: "Asynchron und kollaborativ",
    ...
    stops: [
      {
        ...
        source: "Bilanztool + Enzensberger"
      },
      {
        title: "Solo-Etappe 1",
        subtitle: "Enzensberger im Eigenstudium",
      ...
      {
        title: "Solo-Etappe 2",
        subtitle: "Abgefahren Kap. 4+5",
        focus: "Bewegungsdruck und Hyperdorf vergleichen.",
        textLoop: "Kapitel in getrennter Reihenfolge lesen und vergleichen.",
        tasks: [
      ...
      {
        title: "Co-Travel",
        subtitle: "48h gemeinsam weiterreisen",
        focus: "Zwei Lernende teilen dieselbe Etappe mit unterschiedlichen Rollen.",
        textLoop: "Abgefahren Kap. 6 oder 7 + Wiedmer.",
        tasks: [
          "Jede Person schreibt dieselbe Szene aus zwei Perspektiven.",
          "Gemeinsame Dissensliste erstellen.",
          "Eine Frage ans Plenum stellen."
        ],
        encounter: "Synchron oder asynchron, aber mit Rollenwechsel.",
        output: "Doppelpost.",
        source: "Abgefahren Kap. 6/7 + Flut"
      },
      {
        title: "Nachtzug",
  ...
  },
  {
    ...
    stops: [
      {
        title: "Startbahnhof",
        subtitle: "Gemeinsame Ausgangsthese",
        focus: "Was suchen wir beim Reisen wirklich?",
      ...
      },
      {
        ...
        encounter: "Dialog in wechselnden Duos.",
        output: "Rollenpost.",
        source: "Abgefahren Kap. 4"
      },
      {
        title: "Marktplatz",
        subtitle: "Abgefahren Kap. 5",
      ...
      },
      {
        ...
        encounter: "Kurztreffen wie auf Durchreise.",
        output: "Paradox-Satz.",
      ...
      },
      {
        ...
        encounter: "Peer-Review wie letzte Mitreise.",
        output: "Abschlusspost.",
        source: "Enzensberger + alle"
      }
    ]
  }
];
...
const counterArguments = [
  "Tourismus kann lokale Einkommen sichern, wenn Regeln die Lasten begrenzen.",
  "Nicht jede Reise hat dieselbe Wirkung; Taktung und Verhalten sind entscheidend.",
  "Austausch bleibt wertvoll, wenn er nicht nur Kulissenkonsum ist.",
  "Politische Steuerung ist wirksamer als reine Verbotsmoral.",
...
const els = {
  ...
  thesisBtn: document.querySelector("#thesisBtn"),
  ...
  encounterBtn: document.querySelector("#encounterBtn"),
...
function inferWiedmerAnchor(text) {
  if (text.includes("stagen") || text.includes("dorf") || text.includes("umbau")) {
...
function pickFeedbackThemes(entryText) {
  const scored = feedbackThemes
    .map((theme) => {
      const score = theme.keywords.reduce((sum, keyword) => {
        return sum + (entryText.includes(keyword) ? 1 : 0);
      }, 0);
  ...
  if (scored[0].score === 0) {
    return [feedbackThemes[0], feedbackThemes[1]];
  }

  const primary = scored[0];
  const secondary = scored.find((theme) => theme.id !== primary.id && theme.score > 0) || scored[1] || primary;
  return [primary, secondary];
}

function buildAutoFeedback(entry) {
  const rawText = `${entry.title} ${entry.observation} ${entry.reference} ${entry.stop} ${entry.route}`.toLowerCase();
  ...
  const strength =
    `Dein Beitrag ist stark, weil er ${primaryTheme.strength}. ` +
    `Du formulierst damit nicht nur ein Stimmungsbild, sondern ein analytisches Problem mit gesellschaftlicher Reichweite.`;

  const textBridge =
    `${wiedmerAnchor} ` +
    `Bei Enzensberger wird daraus das Kernparadoxon: Der touristische Zugriff bedroht genau das, was er als Ferne und Erholung verspricht. ` +
    `${groebnerAnchor}`;
  ...
  const impulse =
    ...
    `Bleibe mutig in der Zuspitzung, aber belege jede Zuspitzung sichtbar am Text.`;
...
function isFeedbackComplete(feedback) {
  return (
    feedback &&
    typeof feedback.strength === "string" &&
    typeof feedback.textBridge === "string" &&
    typeof feedback.critical === "string" &&
    typeof feedback.impulse === "string" &&
...
function normalizeBlogEntry(rawEntry) {
  const base = {
  ...
  if (isFeedbackComplete(rawEntry?.feedback)) {
    base.feedback = {
      ...rawEntry.feedback,
...
function routeProgress(routeId) {
  if (!progressState[routeId]) {
    progressState[routeId] = {};
  }
  return progressState[routeId];
}

function isStopDone(routeId, index) {
  const map = progressState[routeId] || {};
  return Boolean(map[index]);
}

function setStopDone(routeId, index, done) {
  const map = routeProgress(routeId);
  map[index] = done;
  saveProgress();
}
...
function renderRouteTracks() {
  ...
  routeCatalog.forEach((route) => {
    ...
    button.type = "button";
    button.className = "route-track";
    button.innerHTML = `<strong>${escapeHtml(route.name)}</strong><span>${escapeHtml(route.profile)}</span>`;
    button.classList.toggle("active", route.id === activeRouteId);
    button.addEventListener("click", () => setActiveRoute(route.id));
    fragment.append(button);
  });

  els.routeTracks.innerHTML = "";
  els.routeTracks.append(fragment);
}

function renderRouteMeta() {
  const route = currentRoute();
  const doneCount = Object.values(progressState[route.id] || {}).filter(Boolean).length;
  const total = route.stops.length;

  els.routeMeta.textContent =
    `${route.name} | Setting: ${route.setting} | Rhythmus: ${route.rhythm} | Fortschritt: ${doneCount}/${total} Etappen`; 
}
...
function renderRouteNav() {
  ...
  route.stops.forEach((stop, index) => {
    ...
    button.innerHTML = `<strong>${index + 1}. ${escapeHtml(stop.title)}</strong><span>${escapeHtml(stop.subtitle)}</span>`;

    if (isStopDone(route.id, index)) {
    ...
    button.addEventListener("click", () => setActiveStop(index));
    fragment.append(button);
  });
...
function renderRouteDetail() {
  ...
  const toggle = document.querySelector("#toggleDone");
  if (toggle) {
...
function initParadoxConsole() {
  els.thesisText.textContent = randomFrom(paradoxTheses);
  els.counterText.textContent = randomFrom(counterArguments);

  els.thesisBtn.addEventListener("click", () => {
    els.thesisText.textContent = randomFrom(paradoxTheses);
  });
...
function calcTravelProfile() {
  ...
  const longFlights = numberOf(els.longFlights);
  const sehnsucht = numberOf(els.motSehnsucht);
  const rest = numberOf(els.motRest);
  const stressFeeling = numberOf(els.motStress);
  const display = numberOf(els.motDisplay);
  const encounter = numberOf(els.motEncounter);

  const gain = clamp(sehnsucht * 2 + rest * 2.2 + encounter * 1.8 - stressFeeling * 0.4, 0, 100);
  const stress = clamp(stressFeeling * 5 + trips * 1.8 + shortFlights * 2 + longFlights * 3 + display * 2.1, 0, 100);
  const local = clamp(trips * 2.6 + shortFlights * 1.7 + longFlights * 2.6 + display * 2.4, 0, 100);
  const eco = clamp(shortFlights * 8 + longFlights * 20 + trips * 1.2, 0, 100);

  els.gainScore.textContent = Math.round(gain);
...
function loadBlogEntries() {
  try {
    const raw = localStorage.getItem(blogStorageKey);
    if (!raw) return;
    const parsed = JSON.parse(raw);
...
function renderFeedbackBlock(entry) {
  if (!entry.feedback) {
    return "";
  }
...
function renderTeacherComments(comments) {
  if (!comments.length) {
    return "<p class='meta'>Noch kein Lehrerkommentar.</p>";
  }

  const items = comments
    .map((comment) => {
      return `<li><strong>${new Date(comment.createdAt).toLocaleString("de-CH")}:</strong> ${formatMultiline(comment.text)}</li>`;
    })
    .join("");

  return `<ul class="teacher-comments">${items}</ul>`;
}

function deleteBlogEntry(id) {
  blogEntries = blogEntries.filter((entry) => entry.id !== id);
  saveBlogEntries();
  renderBlogEntries();
}
...
function renderBlogEntries() {
  ...
  els.blogEntries.innerHTML = blogEntries
    .map(
      (entry) => `
      <article class="blog-entry">
        <h3>${escapeHtml(entry.title)}</h3>
        <p class="meta">${escapeHtml(entry.route)} | ${escapeHtml(entry.stop)} | ${escapeHtml(entry.reference)} | ${new Date(entry.createdAt).toLocaleString("de-CH")}</p>
  ...
  const buttons = els.blogEntries.querySelectorAll("button[data-delete-id]");
  buttons.forEach((button) => {
    button.addEventListener("click", () => {
      deleteBlogEntry(button.getAttribute("data-delete-id"));
    });
  });
...
function populateBlogStops() {
  const route = currentRoute();
  els.blogStop.innerHTML = route.stops
...
function addTeacherComment(entryId, text) {
  const trimmed = text.trim();
  if (!trimmed) return;

  const entry = blogEntries.find((candidate) => candidate.id === entryId);
  if (!entry) return;
...
function regenerateFeedback(entryId) {
  const entry = blogEntries.find((candidate) => candidate.id === entryId);
  if (!entry) return;
...
function renderTeacherPanel() {
  ...
  els.teacherPanel.innerHTML = blogEntries
    .map((entry) => {
      return `
      <article class="teacher-entry">
        <p class="label">Lehrerversion</p>
        <h3>${escapeHtml(entry.title)}</h3>
  ...
  const addButtons = els.teacherPanel.querySelectorAll("button[data-add-comment-id]");
  addButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const entryId = button.getAttribute("data-add-comment-id");
      const input = els.teacherPanel.querySelector(`textarea[data-comment-input-id="${entryId}"]`);
      if (!input) return;
      addTeacherComment(entryId, input.value);
    });
  });

  const regenerateButtons = els.teacherPanel.querySelectorAll("button[data-regenerate-feedback-id]");
  regenerateButtons.forEach((button) => {
    button.addEventListener("click", () => {
      const entryId = button.getAttribute("data-regenerate-feedback-id");
      regenerateFeedback(entryId);
    });
  });
}

function syncTeacherModeUi() {
  if (!els.teacherModeBtn || !els.teacherModeState) return;

  els.teacherModeBtn.setAttribute("aria-pressed", String(isTeacherMode));
...
function initTeacherMode() {
  if (!els.teacherModeBtn) return;

  try {
    const raw = localStorage.getItem(teacherModeStorageKey);
    isTeacherMode = raw === "true";
  } catch {
    isTeacherMode = false;
  }
